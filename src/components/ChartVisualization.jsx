...
const ChartVisualization = ({ articles }) => {
  ...
  const CustomTooltip = ({ active, payload, label }) => {
    if (active && payload && payload.length) {
      ...
      const displayLabel = data.category || data.symbol || label || 'Value';
      ...
      return (
        <motion.div
          ...
          className="bg-gray-800 p-4 border rounded-xl shadow-xl border-gray-700"
        >
          <p className="font-semibold text-white">{`${displayLabel}: ${displayValue}`}</p>
          {data.percentage && (
            <p className="text-sm text-gray-400">{`${data.percentage}% of total`}</p>
          )}
        </motion.div>
      );
    }
  ...
  const StatCard = ({ icon: Icon, title, value, subtitle, gradient, delay }) => (
    ...
    >
      <div className="absolute top-0 right-0 w-24 h-24 sm:w-32 sm:h-32 bg-gray-300 opacity-10 rounded-full -translate-y-12 sm:-translate-y-16 translate-x-12 sm:translate-x-16"></div>
    ...
    >
      ...
      <div className="grid lg:grid-cols-2 gap-6 sm:gap-8 px-4 sm:px-0">
        ...
        <motion.div
          ...
          className="bg-gray-800/50 backdrop-blur-sm rounded-2xl shadow-xl p-4 sm:p-8 border border-gray-700"
        >
          <div className="flex items-center mb-4 sm:mb-6">
            ...
            <div>
              <h3 className="text-lg sm:text-xl font-bold text-white">Category Distribution</h3>
              <p className="text-gray-300 text-xs sm:text-sm">Content breakdown</p>
            </div>
          </div>
            ...
            {categoryData.map((item, index) => (
              ...
              >
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-400">{item.count} articles</p>
                </div>
              </motion.div>
            ))}
          </div>
        ...
        >
          <div className="flex items-center mb-4 sm:mb-6">
            ...
            <div>
              <h3 className="text-lg sm:text-xl font-bold text-white">Top Symbols</h3>
              <p className="text-gray-300 text-xs sm:text-sm">Most mentioned companies</p>
            </div>
          </div>
          
          {symbolData.length > 0 ? (
            <ResponsiveContainer width="100%" height={280}>
              <BarChart data={symbolData} margin={{ top: 20, right: 30, left: 20, bottom: 5 }} style={{ backgroundColor: 'transparent' }}>
      ...
      {timelineChartData.length > 0 && (
        <motion.div
          ...
          className="bg-gray-800/50 backdrop-blur-sm rounded-2xl shadow-xl p-4 sm:p-8 border border-gray-700 mx-4 sm:mx-0"
        >
          <div className="flex items-center mb-4 sm:mb-6">
            ...
            <div>
              <h3 className="text-lg sm:text-xl font-bold text-white">Activity Timeline</h3>
              <p className="text-gray-300 text-xs sm:text-sm">News volume over time</p>
            </div>
          </div>
          
          <ResponsiveContainer width="100%" height={200}>
            <AreaChart data={timelineChartData} style={{ backgroundColor: 'transparent' }}>
              <CartesianGrid strokeDasharray="3 3" stroke="#374151" />
              <XAxis 
                dataKey="date" 
                tick={{ fontSize: window.innerWidth > 640 ? 12 : 10, fill: '#9ca3af' }}
                axisLine={{ stroke: '#4b5563' }}
                tickLine={{ stroke: '#4b5563' }}
